(ns dynamics365.main
  "Kotodama WASM entrypoint for the Dynamics365 clean-room actor (L4) — Clojure port.
  ...
  facts (`dynamics365.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dynamics365")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dynamics365.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dynamics365-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dynamics365-compat/ready)
